...
public class ShareHelper implements IShare, WbShareCallback {
    ...
    private ShareHelper(Build build) {
        ...
        if (!TextUtils.isEmpty(WeiboAppId)) {
            ...
            shareHandler = new WbShareHandler((BaseActivity) mContext);
            shareHandler.registerApp();
            shareHandler.doResultIntent(((BaseActivity) mContext).getIntent(), this);
        }
    }
    ...
    public void onWbShareSuccess() {
        UIToast.showMessage("分享成功");
    }

    @Override
    public void onWbShareCancel() {
        UIToast.showMessage("分享取消");
    }

    @Override
    public void onWbShareFail() {
        UIToast.showMessage("分享失败");
    }
